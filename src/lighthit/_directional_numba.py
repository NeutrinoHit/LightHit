...
def _apply(points, receivers, looks, channels, starts, degree, max_m, max_out,
           alpha, pair_lambda, pair_mu, log_r, coefficients, absorption,
           velocity, omega, flight, start_norm, jmax, receiver_block):
    """Equation (*) for every (receiver, cell) pair. Returns (receiver, freq, 2)."""
    nfreq = len(omega)
    nrec = receivers.shape[0]
    ncell = points.shape[0]
                ...
                for ell in range(degree + 1):
                    ...
                    for a in range(max_out - min(ell, max_out),
                        ...
                        for b in range(max_m - keep, max_m + keep + 1):
                            ...
                            for w in range(nfreq):
                                rotated[a, w] += factor * channels[cell, column, w]
...
class PreparedDirectionalKernel:
    ...
    def apply(self, source, receivers_m, look_directions, alpha, *,
        ...
        result = _apply(local_points, local_receivers, local_looks, data, starts,
                        int(self.degree), int(max_m), int(self.acceptance_degree),
                        alpha, self.pair_lambda, self.pair_mu, self.log_r,
                        self.coefficients, self.absorption_per_m,
                        self.speed_m_per_ns, self.omega_per_ns,
                        self.radial_phase == "flight", _start_norm_table(jmax),
                        int(jmax), int(receiver_block))
